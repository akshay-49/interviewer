...
def build_graph_coach():
    ...
    builder.add_edge("ask", "await_answer")
    builder.add_edge("await_answer", "evaluate")
    # Process decision first to increment question count
    builder.add_edge("evaluate", "decide")
    # After proceed, continue with transition
    builder.add_edge("await_continue", "transition")
    builder.add_edge("transition", "ask")

    # Conditional: decide node routes to either end or await_continue (pause for feedback)
    builder.add_conditional_edges(
        "decide",
        lambda s: "end" if s["end_interview"] else "await_continue",
        {
            "end": "end",
