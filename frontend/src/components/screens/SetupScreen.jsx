...
const SetupScreen = () => {
    ...
    const handleInputChange = (e) => {
        ...
        if (name === 'role') {
            setShowRoleDropdown(value.length > 0);
        } else if (name === 'experience') {
            setShowExperienceDropdown(value.length > 0);
        }
    };
    ...
    const filteredExperiences = experiences.filter(exp => 
        exp.toLowerCase().includes(formData.experience.toLowerCase())
    );
                                    ...
                                    <div className="relative" ref={roleDropdownRef}>
                                        <div className="flex items-center gap-2">
                                            <input
                                                ...
                                                onFocus={() => formData.role && setShowRoleDropdown(true)}
                                                placeholder="e.g., Frontend Developer..."
                                                autoComplete="off"
                                                className="flex-1 px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 focus:ring-2 focus:ring-primary focus:border-transparent text-xs sm:text-sm"
                                            />
                                            <button
                                                type="button"
                                                onClick={() => setShowRoleDropdown(!showRoleDropdown)}
                                                className="px-2 py-2 text-gray-400 hover:text-primary transition-colors"
                                            >
                                                <span className="material-symbols-outlined text-lg">arrow_drop_down</span>
                                            </button>
                                        </div>
                                        {showRoleDropdown && (
                                            <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
                                                {filteredRoles.length > 0 ? (
                                                    filteredRoles.map(role => (
                                                        <button
                                                            ...
                                                            className="w-full text-left px-3 py-2 text-xs sm:text-sm hover:bg-primary/10 dark:hover:bg-primary/20 transition-colors text-gray-900 dark:text-white"
                                                        >
                                                            {role}
                                                        </button>
                                                    ))
                                                    ...
                                                    formData.role && (
                                                        <button
                                                            type="button"
                                                            onClick={() => handleSelectRole(formData.role)}
                                                            className="w-full text-left px-3 py-2 text-xs sm:text-sm hover:bg-primary/10 dark:hover:bg-primary/20 transition-colors text-gray-900 dark:text-white"
                                                        >
                                                            {formData.role}
                                                        </button>
                                    ...
                                    <div className="relative" ref={experienceDropdownRef}>
                                        <div className="flex items-center gap-2">
                                            <input
                                                ...
                                                onChange={handleInputChange}
                                                onFocus={() => formData.experience && setShowExperienceDropdown(true)}
                                            ...
                                            <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
                                                {filteredExperiences.length > 0 ? (
                                                    ...
                                                    formData.experience && (
                                                        <button
                                                            type="button"
                                                            onClick={() => handleSelectExperience(formData.experience)}
                                                            className="w-full text-left px-3 py-2 text-xs sm:text-sm hover:bg-primary/10 dark:hover:bg-primary/20 transition-colors text-gray-900 dark:text-white"
                                                        >
                                                            {formData.experience}
                                                        </button>
